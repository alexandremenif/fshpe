...
case class DepthFirstSolver[S](problem: SubProblem[S], maxDepth: Long = Long.MaxValue) extends Solver[S] {
  ...
  override def subProblems: Stream[SubProblem[S]] = subProblems(problem, maxDepth)

  private def subProblems(subProblem: SubProblem[S], maxDepth: Long): Stream[SubProblem[S]] = {
    if (maxDepth > 0)
